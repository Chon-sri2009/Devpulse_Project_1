...
try {
    ...
    if (!$location.Contains('streaming') -or !$location.Contains('user-read-email') -or !$location.Contains('user-library-read') -or !$location.Contains('playlist-read-private') -or !$location.Contains('playlist-read-collaborative')) { throw 'Spotify browser/library OAuth scopes missing.' }
    $r.Dispose()
    $request.Dispose()
